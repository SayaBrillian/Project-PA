<template>
  <q-form ref="formRef" class="auth-form" @submit.prevent="registerUser">
    ...
    <q-input v-model="username" outlined hide-bottom-space lazy-rules maxlength="30" label="Username"
      ...
      <template #append>
        <q-btn flat round dense :loading="checkingUsername"
          :icon="usernameChecked ? (usernameAvailable ? 'task_alt' : 'cancel') : 'task_alt'"
          :color="usernameChecked ? (usernameAvailable ? 'positive' : 'negative') : 'grey'" @click="checkUsername" />
      </template>
    </q-input>
    <!-- Tanggal Lahir -->
    ...
    <!-- Email -->
    <q-input v-model="email" outlined clearable hide-bottom-space lazy-rules maxlength="100" type="email" label="Email"
      placeholder="contoh@email.com" :rules="emailRules">
      <template #prepend>
        <q-icon name="mail" />
      </template>
      <template #append>
        <q-btn flat round dense :disable="!email" :loading="checkingEmail"
          :icon="emailChecked ? (emailAvailable ? 'task_alt' : 'cancel') : 'task_alt'"
          :color="emailChecked ? (emailAvailable ? 'positive' : 'negative') : 'grey'" @click="checkEmail" />
      </template>
    </q-input>
    <!-- Nomor HP -->
    <q-input v-model="phone" outlined clearable hide-bottom-space lazy-rules maxlength="20" type="tel" label="Nomor HP"
      placeholder="08xxxxxxxxxx" :rules="phoneRules">
      <template #prepend>
        <q-icon name="call" />
      </template>
      <template #append>
        <q-btn flat round dense :disable="!phone" :loading="checkingPhone"
          :icon="phoneChecked ? (phoneAvailable ? 'task_alt' : 'cancel') : 'task_alt'"
          :color="phoneChecked ? (phoneAvailable ? 'positive' : 'negative') : 'grey'" @click="checkPhone" />
      </template>
    </q-input>
    <!-- Password -->
    <q-input v-model="password" outlined hide-bottom-space lazy-rules maxlength="12"
    ...
    <!-- Konfirmasi Password -->
    <q-input v-model="confirmPassword" outlined hide-bottom-space lazy-rules maxlength="12"
...
|--------------------------------------------------------------------------
*/
const CHECK_URL = '/api/user/check'
...
const passwordHint = 'Password minimal 8 karakter, maksimal 12 karakter, mengandung huruf besar, huruf kecil, dan angka.'
...
async function registerUser() {
  ...
  if (
    ...
    $q.notify({
      type: 'warning',
      message: 'Isi Email atau Nomor HP.'
    })
    return
  }
